	...
	dataAction(){
		...
		var collectionForm = function(obj){
			...
			for(var prop in obj){
				...
				if(prop.indexOf('[]') > -1){
					_.extend(child,{[prop]:obj[prop]});
				}else{
					_.extend(parent,{[prop]:obj[prop]});
				}
			}
		...
		var parentPropOpt = function(obj){
		 ...
		 */
		var cssHtml = function(compate,key,val,isVal){
			var prefixes = [];
			if(compate == "new"){
				prefixes = ['-webkit-',''];
			}else if(compate == "old"){
		...
		var compateParentOpt = function(obj){
			var arr = [".flex-container{"];
			for(var prop in obj){
				if(version_new){
					if(prop == "display"){
		...
		var compateChildOpt = function(obj){
			var keys = _.keys(obj);
			var len = _.values(obj)[0].length; // 子项的数量,取第一个属性值的长度
			var arr = [];
			var itemsArr = [];
